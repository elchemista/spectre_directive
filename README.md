...
def deps do
  [
    {:spectre, "~> 0.3.0"},
    {:spectre_directive, github: "elchemista/spectre_directive", branch: "main"}
  ]
end
```
...
Directive exposes its continuity configuration through the package-local Stack
DSL:
...
Spectre owns subject continuity through one `Spectre.Instance` addressed
...
separate core facilities and are not duplicated by Directive.
